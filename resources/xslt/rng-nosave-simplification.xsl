...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://relaxng.org/ns/structure/1.0" xmlns:rng="http://relaxng.org/ns/structure/1.0" xmlns:exsl="http://exslt.org/common" version="1.0" extension-element-prefixes="exsl" exclude-result-prefixes="exsl rng">
...
<!-- zap anything in foreign NSs /-->
  ...
  <xsl:template match="*|@*" mode="step7.2" priority="-2"/>
<!-- 7.3 -->
  <xsl:template match="/" mode="step7.3">
    <xsl:variable name="step3">
      <xsl:apply-templates mode="step7.3"/>
    </xsl:variable>
    <xsl:apply-templates select="exsl:node-set($step3)" mode="step7.4">
			</xsl:apply-templates>
  </xsl:template>
...
<!-- 7.22 -->
  <xsl:template match="/" mode="step7.22">
    <xsl:variable name="step22">
      <xsl:apply-templates mode="step7.22"/>
    </xsl:variable>
    <xsl:copy-of select="$step22"/>
      <!-- <xsl:apply-templates select="exsl:node-set($step22)" mode="done"></xsl:apply-templates> -->
  </xsl:template>
 ...
 <xsl:template match="*" mode="done" priority="-1000">
   <xsl:copy-of select="."/>
 </xsl:template>
</xsl:stylesheet>
