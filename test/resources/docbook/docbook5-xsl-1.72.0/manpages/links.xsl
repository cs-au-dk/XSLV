...
<xsl:template match="d:ulink|d:footnote[not(ancestor::d:table)]|d:annotation|d:alt">
  ...
  <xsl:variable name="get.links.with.unique.urls">
    ...
    <xsl:copy-of
        select="$all.links.with.unique.urls/refentry.link.set
                [@idref = generate-id(current()/ancestor::d:refentry)]/notesource"/>
  </xsl:variable>
...
<xsl:template match="*" mode="notesource.number">
  <xsl:value-of select="count(preceding::*[(self::d:ulink
                        or self::d:footnote[not(ancestor::d:table)]
                        or self::d:annotation
                        or self::d:alt)
                        and node()
                        ...
                        and not(ancestor::d:indexterm)
                        and (generate-id(ancestor::d:refentry)
                        = generate-id(current()/ancestor::d:refentry))]/@url)]
                        [generate-id(ancestor::d:refentry)
                        = generate-id(current()/ancestor::d:refentry)]) + 1"/>
</xsl:template>
