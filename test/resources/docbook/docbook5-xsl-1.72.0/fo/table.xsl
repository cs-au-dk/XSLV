...
<xsl:template match="d:tgroup" name="tgroup">
  ...
  <xsl:if test="count(preceding-sibling::d:tgroup) = 0">
...
<xsl:template name="table.width">
  ...
  <xsl:variable name="column.sum">
    <xsl:choose>
      <!-- CALS table -->
      <xsl:when test="d:tgroup[1][@cols]">
        <xsl:if test="count(d:tgroup[1]/d:colspec) = d:tgroup/@cols">
          <xsl:for-each select="d:tgroup[1]/d:colspec">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      ...
      <xsl:variable name="cell-orientation">
        <xsl:call-template name="dbfo-attribute">
          <xsl:with-param name="pis"
                          select="ancestor-or-self::d:entry/processing-instruction('dbfo')"/>
      ...
      <xsl:variable name="row-orientation">
        <xsl:call-template name="dbfo-attribute">
          <xsl:with-param name="pis"
                          select="ancestor-or-self::d:row/processing-instruction('dbfo')"/>
          <xsl:with-param name="attribute" select="'orientation'"/>
        </xsl:call-template>
      </xsl:variable>
