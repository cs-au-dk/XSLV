...
xmlns:doc="http://nwalsh.com/xsl/documentation/1.0"
		...
		xmlns:db="http://docbook.org/ns/docbook"
                version="1.0"
                exclude-result-prefixes="doc ng db d">
...
<xsl:template match="d:book" mode="jhtoc">
  ...
  <tocitem target="{$id}">
    ...
    <xsl:apply-templates select="d:part|d:reference|d:preface|d:chapter|d:appendix|d:article|d:colophon"
                         mode="jhtoc"/>
  </tocitem>
</xsl:template>
...
<xsl:template match="d:part|d:reference|d:preface|d:chapter|d:appendix|d:article"
  ...
  <tocitem target="{$id}">
    ...
    <xsl:apply-templates
      select="d:article|d:preface|d:chapter|d:appendix|d:refentry|d:section|d:sect1"
      mode="jhtoc"/>
  </tocitem>
</xsl:template>
...
<xsl:template match="d:part|d:reference|d:preface|d:chapter|d:appendix|d:refentry|d:article"
              mode="map">
  <xsl:variable name="id">
    <xsl:call-template name="object.id"/>
  </xsl:variable>
...
<xsl:template match="d:indexterm" mode="idx">
  ...
  <xsl:choose>
    <xsl:when test="d:see">
      <xsl:variable name="see"><xsl:value-of select="d:see"/></xsl:variable>
