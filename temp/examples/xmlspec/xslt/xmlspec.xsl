...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <h5 xmlns="http://www.w3.org/1999/xhtml">
          <xsl:call-template name="anchor">
            <xsl:with-param name="no_de" select=".."/>
            <xsl:with-param name="conditional" select="0"/>
          </xsl:call-template>
  ...
  <xsl:template match="header">
    <div xmlns="http://www.w3.org/1999/xhtml" class="head">
      ...
      <h1>
        <xsl:call-template name="anchor">
          <xsl:with-param name="no_de" select="title[1]"/>
          <xsl:with-param name="conditional" select="0"/>
      ...
      <xsl:if test="subtitle">
        ...
        <h2>
          <xsl:call-template name="anchor">
            <xsl:with-param name="no_de" select="subtitle[1]"/>
            <xsl:with-param name="conditional" select="0"/>
      ...
      <h2>
        <xsl:call-template name="anchor">
          <xsl:with-param name="no_de" select="w3c-doctype[1]"/>
          <xsl:with-param name="conditional" select="0"/>
...
<!--
  ...
  <xsl:template name="list.numeration">
    <xsl:param name="no_de" select="."/>

    <xsl:choose>
      <xsl:when test="$no_de/ancestor::olist">
        <xsl:call-template name="next.numeration">
          <xsl:with-param name="numeration">
            <xsl:call-template name="list.numeration">
              <xsl:with-param name="no_de" select="$no_de/ancestor::olist[1]"/>
            </xsl:call-template>
          </xsl:with-param>
        </xsl:call-template>
      </xsl:when>
  ...
  <xsl:template match="olist">
    ...
    <ol xmlns="http://www.w3.org/1999/xhtml" class="enum{$type}">
      <xsl:apply-templates/>
    </ol>
  </xsl:template>
...
<xsl:template name="object.id">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$no_de/@id">
      <xsl:value-of select="$no_de/@id"/>
    </xsl:when>
    <xsl:otherwise>
      <xsl:value-of select="generate-id($no_de)"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template name="anchor">
  ...
  <xsl:variable name="id">
    <xsl:call-template name="object.id">
      <xsl:with-param name="no_de" select="$no_de"/>
      <xsl:with-param name="default.id" select="$default.id"/>
    </xsl:call-template>
  </xsl:variable>
  <xsl:if test="$conditional = 0 or $no_de/@id">
