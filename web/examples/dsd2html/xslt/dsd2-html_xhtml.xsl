...
be changed is required to make more universal. --><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xi="http://www.w3.org/2001/XInclude" xmlns:m="http://www.brics.dk/DSD/2.0/meta" xmlns:dsd="http://www.brics.dk/DSD/2.0" xmlns:doc="http://russet.eris.qinetiq.com/Library/DSDs/doc" xmlns:list="http://russet.eris.qinetiq.com/Library/DSDs/list" xmlns:copyright="http://russet.eris.qinetiq.com/Library/DSDs/copyright" xmlns:address="http://russet.eris.qinetiq.com/Library/DSDs/address" xmlns:postcode="http://russet.eris.qinetiq.com/Library/DSDs/postcode" xmlns:person="http://russet.eris.qinetiq.com/Library/DSDs/person" xmlns:t="http://russet.eris.qinetiq.com/Library/DSDs/text" xmlns:vers="http://russet.eris.qinetiq.com/Library/DSDs/vers" xmlns:link="http://russet.eris.qinetiq.com/Library/DSDs/link" xmlns="http://www.w3.org/1999/xhtml" version="1.0">
...
<xsl:template match="dsd">
	<html>
		<head>
			...
			<title><xsl:apply-templates select="//doc:title" mode="title"/></title>
			<style type="text/css">
		...
		<body>
			<xsl:apply-templates select="doc" mode="headings"/><xsl:text disable-output-escaping="yes">&lt;hr&gt;</xsl:text>
			<xsl:call-template name="processRules"/><xsl:text disable-output-escaping="yes">&lt;hr&gt;</xsl:text>
...
<xsl:template name="processRules">
   <xsl:variable name="indentLevel" select="0"/>
	<xsl:variable name="block-indentation" select="'background-color: beige; text-indent:{$indentLevel}ex;'"/>
	<xsl:variable name="condition-block-indentation" select="'background-color: pink; text-indent:{$indentLevel}ex;'"/>
	<xsl:for-each select="*[not(self::doc) and not(self::import)]">
		...
		<div style="font-family:Courier; font-weight:bold; margin-left=1pt; margin-right=1pt; margin-bottom:4pt; margin-top:1pt; border-style: solid; border-width: 2px;  border-color:pink;">
		<xsl:choose>
			<xsl:when test="(    name()='rule' or   name()='contenttype' or   name()='stringtype' or   name()='boolexp'  ) and @id">
				<xsl:call-template name="definitions">
					<xsl:with-param name="indentLevel" select="$indentLevel"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="(    name()='and' or   name()='or' or   name()='not' or   name()='imply' or   name()='equiv' or   name()='one' or   name()='parent' or   name()='ancestor' or   name()='child' or   name()='descendant' or   name()='this' or   name()='root' or   name()='element' or   name()='attribute' or   name()='contents' or   name()='boolexp' )">
				<xsl:call-template name="boolexp-definitions">
					<xsl:with-param name="indentLevel" select="$indentLevel"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="(    name()='unique' or   name()='pointer' or   name()='if' or   name()='rule' or   name()='declare' or   name()='require' )">
...
<xsl:template name="output-entry-title">
	<div style="margin-left=1pt; margin-right=1pt; margin-bottom:1pt; margin-top:15pt; border-width: 2px;">
...
<xsl:template match="doc" mode="definitions">
	<xsl:apply-templates select="doc:desc" mode="definitions"/>
</xsl:template>

<xsl:template match="doc:desc" mode="definitions">
	...
	<xsl:choose>
		...
		<xsl:when test="name(.)='unique' or name()='pointer'">
			<xsl:call-template name="unique">
				<xsl:with-param name="indentLevel" select="$indentLevel + 2.7"/>
			</xsl:call-template>
		</xsl:when>
		<xsl:otherwise/>
	</xsl:choose>
...
<xsl:template name="rules">
   <xsl:param name="indentLevel"/>
	<xsl:element name="div" namespace="http://www.w3.org/1999/xhtml">
		<xsl:attribute name="style">background-color: beige; text-indent:<xsl:value-of select="$indentLevel"/>ex;</xsl:attribute>
		<xsl:choose>
			<xsl:when test="name(.)='declare'">
...
<xsl:template name="contents-decl">
   <xsl:param name="indentLevel"/>
	<xsl:element name="div" namespace="http://www.w3.org/1999/xhtml">
		...
		<xsl:for-each select="*">
			<xsl:choose>
				<xsl:when test="   name()='sequence' or   name()='optional' or   name()='complement' or   name()='union' or   name()='intersection' or   name()='minus' or   name()='repeat' or   name()='string' or   name()='char' or   name()='stringtype' or   name()='contenttype' or    name()='and' or   name()='or' or   name()='not' or   name()='imply' or   name()='equiv' or   name()='one' or   name()='parent' or   name()='ancestor' or   name()='child' or   name()='descendant' or   name()='this' or   name()='root' or   name()='element' or   name()='attribute' or   name()='contents' or   name()='boolexp'">
					<xsl:call-template name="regexp">
						<xsl:with-param name="indentLevel" select="$indentLevel + 2.7"/>
					</xsl:call-template>
				</xsl:when>
...
<xsl:template name="contents-default">
   <xsl:param name="indentLevel"/>
	<xsl:element name="div" namespace="http://www.w3.org/1999/xhtml">
		<xsl:attribute name="style">background-color: beige; text-indent:<xsl:value-of select="$indentLevel"/>ex;</xsl:attribute>
		<xsl:call-template name="render-element-name"/><xsl:text disable-output-escaping="yes">&lt;br&gt;</xsl:text>
		<xsl:element name="div" namespace="http://www.w3.org/1999/xhtml">
			<xsl:attribute name="style">background-color: silver; text-indent:<xsl:value-of select="$indentLevel + 2.7"/>ex;</xsl:attribute>
			<xsl:for-each select="*">
				<xsl:choose>
...
<xsl:template name="boolexp-definitions">
	...
	<xsl:element name="div" namespace="http://www.w3.org/1999/xhtml">
		<xsl:attribute name="style">
			background-color: beige; text-indent:<xsl:value-of select="$indentLevel"/>ex;
		</xsl:attribute>
		<xsl:choose>
...
<xsl:template name="boolexp">
	...
	<xsl:element name="div" namespace="http://www.w3.org/1999/xhtml">
		<xsl:attribute name="style">
			<xsl:choose>
				<xsl:when test="$isCondition">background-color: pink; text-indent:<xsl:value-of select="$indentLevel"/>ex;</xsl:when>
				<xsl:otherwise>background-color: beige; text-indent:<xsl:value-of select="$indentLevel"/>ex;</xsl:otherwise>
			</xsl:choose>
		</xsl:attribute>
		<xsl:choose>
...
<xsl:template match="doc" mode="headings">
	<h1 class="titleHeader"><xsl:value-of select="doc:title"/></h1>
	<table border="0">
		<tr align="left">
	...
	<span class="header"><b>Description</b></span><xsl:text disable-output-escaping="yes">&lt;br&gt;</xsl:text><xsl:text disable-output-escaping="yes">&lt;br&gt;</xsl:text>
	<xsl:apply-templates select="doc:desc" mode="meta-data"/><xsl:text disable-output-escaping="yes">&lt;p&gt;</xsl:text>
</xsl:template>
...
<xsl:template match="doc" mode="meta-data">
	<table border="1" cellpadding="2">
		<tr valign="top">
			<td width="55%"><xsl:apply-templates select="doc:versions" mode="meta-data"/></td>
			<td><xsl:apply-templates select="doc:authors" mode="meta-data"/></td>
		</tr>
	</table>
	<xsl:apply-templates select="doc:copyright" mode="meta-data"/>
</xsl:template>

<!-- -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-* -->
<!-- -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*- VERSION NUMBERS -*-*-*-*-*-*-*-*-*-*-*-*-*-*-* -->

<xsl:template match="doc:versions" mode="meta-data">
	<table border="0">
...
<xsl:template match="doc:authors" mode="meta-data">
...
<xsl:template match="email" mode="person-address">
   <tt><xsl:element name="a" namespace="http://www.w3.org/1999/xhtml">
...
<xsl:template match="doc:desc" mode="meta-data">
	<span class="desc"><xsl:apply-templates mode="inline-text"/></span>
</xsl:template>
...
<xsl:template match="doc:copyright" mode="meta-data">
	<xsl:value-of select="leader/@value"/> © 
